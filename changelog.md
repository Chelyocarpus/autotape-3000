...
- **Packaged builds (installer and portable) crashed on launch with a generic "undefined: undefined" error** — the bundler was inlining `@ffmpeg-installer/ffmpeg` instead of leaving it as a real dependency, which broke that package's own `__dirname`-relative lookup of the bundled ffmpeg binary and made it throw a bare string during startup. It's now kept external so it resolves correctly in any packaged build.
