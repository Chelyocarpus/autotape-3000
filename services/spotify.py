"""Windows GSMTC (Global System Media Transport Controls) Spotify integration."""

import asyncio
import threading

# Fallback poll interval used when GSMTC events are unavailable or as a safety net.
SPOTIFY_POLL_INTERVAL_MS = 250
...
async def _gsmtc_get_spotify_info() -> dict | None:
    """Return structured track info from Spotify's GSMTC session, or None."""
...
def _get_spotify_info_sync() -> dict | None:
    """Synchronous wrapper; safe to call from any non-async background thread."""
    try:
        return asyncio.run(_gsmtc_get_spotify_info())
    except Exception:  # noqa: BLE001
        return None
...
async def _gsmtc_event_watcher(emit_fn: callable, stop_event: threading.Event) -> None:
    """
    Subscribe to GSMTC playback/media events and call emit_fn(info) immediately
    on each change.  Falls back to periodic polling (SPOTIFY_POLL_INTERVAL_MS) as
    a safety net and for initial state.
    """
    ...
    interval = SPOTIFY_POLL_INTERVAL_MS / 1000
    ...
    try:
        while not stop_event.is_set():
            ...
            try:
                ...
                trigger.clear()
            info = await _gsmtc_get_spotify_info()
            emit_fn(info)
    finally:
        try:
...
def run_gsmtc_watcher(emit_fn: callable, stop_event: threading.Event) -> None:
    """
    Blocking call that runs the GSMTC event-driven watcher until stop_event is set.
    Must be called from a dedicated background thread.
    Falls back to simple polling when GSMTC is unavailable.
    """
    if not _GSMTC_AVAILABLE:
        while not stop_event.wait(timeout=SPOTIFY_POLL_INTERVAL_MS / 1000):
            emit_fn(_get_spotify_info_sync())
        return
